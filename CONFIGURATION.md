...
| `LAST30DAYS_X_MIN_FAVES` | Integer likes floor appended to the **Apify** X backend's TOPIC search as `min_faves:<N>` (handle/mention lookups are untouched). Default `0`/unset = off, unchanged behaviour. That backend is pay-per-result, so a floor both sharpens a narrow topic's pool and stops paying for small-account noise. Applies to the `apify` backend only — a failover to `bird`/`xai`/`xurl`/`xquik` carries no floor. Set it too high and, when `apify` is your only X backend, the lane returns nothing — a clean empty source, not an error. When another X backend is also configured, an emptied `apify` result is indistinguishable from a dead one and the chain fails over to it, so the floor stops filtering and a second (possibly paid) backend runs on every X fetch. |
